...
public class PoAction {
	...
	public void deletePo(TPo po) {
		if(service.getAllPoDetail(po.getPoId()).size() > 0){
			FacesContext.getCurrentInstance().addMessage("form", new FacesMessage(FacesMessage.SEVERITY_ERROR, "该订单存在明细，无法删除。请删除明细后再删除订单。",null));
			return ;
		}
		service.deletePo(po);
		this.getAllPo();
